    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.waitBox}>
          ...
          <Text style={styles.waitDesc}>School admin is preparing the timetable. Please check back soon!</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.topBar}>
        <View>
          <Text style={styles.header}>Parent Portal</Text>
          <Text style={styles.sub}>Child: Anjali Singh — Class 10-A</Text>
        </View>
        <TouchableOpacity style={styles.logoutBtn} onPress={onLogout}>
          <Text style={styles.logoutText}>Logout</Text>
        </TouchableOpacity>
      ...
      {tab === 'timetable' && (
        <>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {childTimetable[selectedDay].map((item, i) => (
      ...
      {tab === 'ptm' && (
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.ptmCard}>
            <Text style={styles.ptmTitle}>📅 Your PTM Slot</Text>
            <Text style={styles.ptmDate}>Saturday, 20 April 2026</Text>
            <Text style={styles.ptmTime}>⏰ 9:00 AM — 9:45 AM</Text>
            <Text style={styles.ptmNote}>You will meet 5 teachers in 45 minutes{'\n'}No waiting time — fixed slot assigned</Text>
          </View>
          <View style={styles.teacherList}>
            <Text style={styles.teacherListTitle}>Teachers You Will Meet</Text>
            {[
              { time: '9:00', teacher: 'Mr. Kumar', subject: 'Mathematics' },
              { time: '9:09', teacher: 'Mrs. Priya', subject: 'Science' },
              { time: '9:18', teacher: 'Ms. Rani', subject: 'English' },
              { time: '9:27', teacher: 'Mr. Sharma', subject: 'Hindi' },
              { time: '9:36', teacher: 'Mrs. Gupta', subject: 'Social Science' },
            ].map((t, i) => (
              <View key={i} style={styles.teacherRow}>
                <Text style={styles.teacherTime}>{t.time} AM</Text>
                <View style={styles.teacherInfo}>
                  <Text style={styles.teacherName}>{t.teacher}</Text>
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, backgroundColor: '#ffffff', borderRadius: 12, padding: 10, alignItems: 'center', borderWidth: 1.5, borderColor: '#DDD8FF' },
  tabActive: { backgroundColor: '#26215C', borderColor: '#26215C' },
  tabText: { color: '#26215C', fontWeight: '600', fontSize: 12 },
  ...
  ptmCard: { backgroundColor: '#534AB7', borderRadius: 16, padding: 20, marginBottom: 16 },
  ptmTitle: { fontSize: 16, fontWeight: '700', color: '#ffffff', marginBottom: 6 },
  ptmDate: { fontSize: 14, color: '#DDD8FF', marginBottom: 4 },
  ptmTime: { fontSize: 18, fontWeight: '800', color: '#ffffff', marginBottom: 10 },
  ptmNote: { fontSize: 13, color: '#DDD8FF', lineHeight: 20 },
  teacherList: { backgroundColor: '#ffffff', borderRadius: 16, padding: 16, borderWidth: 1.5, borderColor: '#DDD8FF' },
  teacherListTitle: { fontSize: 15, fontWeight: '700', color: '#26215C', marginBottom: 14 },
  teacherRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f0eeff' },
  teacherTime: { fontSize: 13, fontWeight: '700', color: '#534AB7', minWidth: 65 },
  teacherInfo: { flex: 1 },
  teacherName: { fontSize: 14, fontWeight: '700', color: '#26215C' },
  teacherSubject: { fontSize: 12, color: '#7F77DD' },
});